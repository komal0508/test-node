...
module.exports = function(app) {
  ...
  */
 app.post("/api/get-accepted-notification-by-filter", (req, res, next) => {
    ...
    WHERE office_works.is_accepted = true and attendances.factory_name = '${req.body.factoryName}'`, { type: sequelize.QueryTypes.SELECT })
    .then((response) => {
        ...
        if(response && response.length && response.length > 0) {
            res.statusCode = 200;
                     res.send({
                         status: 200,
                         message: 'Accepted notification information fetched!!!',
                         data: response,
                   });
                   res.end();
        } else {
            ...
            res.send({
                status: 201,
                message: 'Accepted notification information not fetched!!!',
                data: response,
          });
          res.end();
        }
