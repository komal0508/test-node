...
module.exports = function(app) {
  ...
  */
 app.post("/api/get-attendance-by-factory-&-date-filter", (req, res, next) => {
    sequelize.query(`SELECT emp_id, date, punch_in, punch_out, total_time FROM public.attendances
    WHERE date = ${req.body.date} and emp_id IN
    ...
    .then((response) => {
        ...
        if(response && response.length && response.length > 0) {
            res.statusCode = 200;
                     res.send({
                         status: 200,
                         message: 'According to factoryName and date data is fetched!!!',
                         data: response,
                   });
                   res.end();
        } else {
            ...
            res.send({
                status: 201,
                message: 'According to factoryName and date data is not fetched!!!',
                data: response,
          });
          res.end();
        }
